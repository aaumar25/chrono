...
pub fn constructWindowsToIANAHashmap(gpa: std.mem.Allocator) !WindowsToIANAHashmap {
    ...
    for (DATA) |datapoint| {
        ...
        var iana_identifier_iter = std.mem.splitScalar(
            u8,
            datapoint.iana_identifiers,
            ' ',
        );
        while (iana_identifier_iter.next()) |iana_identifier_string| {
            const iana_identifier =
                chrono.tz.Identifier.parse(iana_identifier_string) catch
                    unreachable; // We are shipping all the data; it better parse
        ...
        map.putAssumeCapacityNoClobber(
            key,
            try identifiers.toOwnedSlice(gpa),
        );
    }
